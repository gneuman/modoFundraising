      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-20">
          <div className="max-w-3xl">
            <p className="text-(--brand-teal) text-sm font-bold uppercase tracking-widest mb-4">El equipo detrás del programa</p>
            <h1 className="text-5xl md:text-6xl font-black leading-none mb-4 tracking-tight">
              Advisory <span className="text-(--brand-teal)">Board</span>
            </h1>
        ...
        {advisors.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {advisors.map((a) => (
              <div
                key={a.id ?? a.nombre}
                className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-(--brand-teal)/30 hover:bg-(--brand-teal)/5 transition-all flex flex-col gap-4"
              >
                <div className="flex items-center gap-4">
                  {a.foto_url ? (
                    ...
                    <div className="w-14 h-14 rounded-full bg-(--brand-teal)/20 border border-(--brand-teal)/30 flex items-center justify-center text-lg font-black text-(--brand-teal) flex-shrink-0">
                      {initials(a.nombre)}
                    </div>
                  )}
                  <div>
                    ...
                    <div className="text-(--brand-teal) text-xs font-semibold">{a.especialidad}</div>
                  </div>
                </div>
                ...
                <div className="text-white/40 text-xs border-t border-white/10 pt-3 space-y-1">
                  ...
                  {a.pricing_display && <p className="text-(--brand-teal) font-semibold">{a.pricing_display}</p>}
                </div>
                {a.calendly_url && (
                  <a
                    href={a.calendly_url}
                    ...
                    className="text-center text-xs font-bold border border-(--brand-teal)/30 text-(--brand-teal) rounded-lg py-2 hover:bg-(--brand-teal)/10 transition-colors"
                  >
                    Agendar sesión →
                  </a>
                )}
      ...
      <section className="bg-white/5 border-y border-white/10 py-20 px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-black mb-6 text-center">
            Aprende de quienes <span className="text-(--brand-teal)">lo hicieron</span>
          </h2>
