  ...
  return (
    <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
      ...
      >
        ...
        {clase.Portada?.[0] ? (
          <div className="w-16 h-10 rounded-xl overflow-hidden shrink-0 bg-zinc-100">
            <Image
              src={
                clase.Portada[0].thumbnails?.large?.url ?? clase.Portada[0].url
              }
              alt={clase.titulo ?? ""}
              width={64}
              height={40}
      ...
      {(() => {
        ...
        const clasePaso =
          isGrabada ||
          (clase.fecha ? new Date(clase.fecha).getTime() <= ahora : false);
        const recursosVisibles = clase.recursosData.filter(
          (r) =>
            !r.fecha_disponible ||
            new Date(r.fecha_disponible).getTime() <= ahora,
        );
