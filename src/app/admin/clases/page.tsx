...
import { ClasesList } from "@/components/admin/clases-list";
...
export default async function AdminClasesPage() {
  ...
  // Clases con fecha pero a las que les falta evento (principal o de equipo)
  const clasesSinEvento = clases
    .filter((c) => c.fecha && (!c.calendar_event_id || !c.calendar_event_id_team))
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Gestiona clases, misiones y recursos del programa
          </p>
        </div>
        <CalendarSetup hasCalendarId={hasCalendarId} clasessinEvento={clasesSinEvento} />
      </div>
      <ClasesList initialClases={clases} />
    </div>
  );
}
