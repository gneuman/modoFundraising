...
import {
  Mail, Zap, Save, Plus, Trash2, Loader2, ToggleLeft, ToggleRight,
  Eye, EyeOff, CreditCard, UserCheck, UserX, Bell, XCircle, LogIn,
...
const TRIGGER_META: Record<TriggerEvent, {
  label: string;
  description: string;
  ...
  checkout_completed:     { label: "Pago inicial recibido",       description: "Alguien pagó la primera cuota y se unió al programa.",     icon: <CreditCard className="h-4 w-4" />,    color: "text-emerald-600 bg-emerald-50 border-emerald-200", dot: "bg-emerald-500" },
  invoice_paid_cuota2:    { label: "Cuota 2 pagada",              description: "El segundo pago fue procesado correctamente.",             icon: <CreditCard className="h-4 w-4" />,    color: "text-emerald-600 bg-emerald-50 border-emerald-200", dot: "bg-emerald-500" },
  invoice_paid_cuota3:    { label: "Cuota 3 pagada",              description: "El tercer y último pago fue procesado.",                   icon: <CreditCard className="h-4 w-4" />,    color: "text-emerald-600 bg-emerald-50 border-emerald-200", dot: "bg-emerald-500" },
  payment_failed_1:       { label: "Pago fallido — 1er intento",  description: "El cobro automático falló por primera vez.",              icon: <CreditCard className="h-4 w-4" />,    color: "text-orange-600 bg-orange-50 border-orange-200",   dot: "bg-orange-400" },
  payment_failed_2:       { label: "Pago fallido — 2do intento",  description: "Segundo intento de cobro fallido.",                       icon: <CreditCard className="h-4 w-4" />,    color: "text-orange-600 bg-orange-50 border-orange-200",   dot: "bg-orange-400" },
  payment_failed_3:       { label: "Pago fallido — 3er intento",  description: "Tercer fallo. Se suspende el acceso.",                    icon: <XCircle className="h-4 w-4" />,       color: "text-red-600 bg-red-50 border-red-200",            dot: "bg-red-500" },
  admission_approved:     { label: "Admisión aprobada",           description: "Aprobaste manualmente a un postulante.",                  icon: <UserCheck className="h-4 w-4" />,     color: "text-blue-600 bg-blue-50 border-blue-200",         dot: "bg-blue-500" },
  admission_rejected:     { label: "Postulación rechazada",       description: "Rechazaste manualmente a un postulante.",                 icon: <UserX className="h-4 w-4" />,         color: "text-zinc-600 bg-zinc-100 border-zinc-200",        dot: "bg-zinc-400" },
  follow_up_1:            { label: "Follow-up admisión 1",        description: "Recordatorio para postulantes que no respondieron.",      icon: <Bell className="h-4 w-4" />,          color: "text-violet-600 bg-violet-50 border-violet-200",   dot: "bg-violet-500" },
  follow_up_2:            { label: "Follow-up admisión 2",        description: "Segundo recordatorio sin respuesta.",                     icon: <Bell className="h-4 w-4" />,          color: "text-violet-600 bg-violet-50 border-violet-200",   dot: "bg-violet-500" },
  subscription_cancelled: { label: "Suscripción cancelada",       description: "El alumno canceló desde el portal.",                     icon: <XCircle className="h-4 w-4" />,       color: "text-red-600 bg-red-50 border-red-200",            dot: "bg-red-500" },
  portal_deactivated:     { label: "Acceso suspendido",           description: "El portal fue desactivado por falta de pago.",           icon: <XCircle className="h-4 w-4" />,       color: "text-red-600 bg-red-50 border-red-200",            dot: "bg-red-500" },
  application_received:   { label: "Postulación recibida",        description: "Alguien completó el formulario de postulación.",         icon: <MessageSquare className="h-4 w-4" />, color: "text-blue-600 bg-blue-50 border-blue-200",         dot: "bg-blue-500" },
  onboarding:             { label: "Bienvenida al portal",        description: "El alumno activó su cuenta y entró al portal.",          icon: <LogIn className="h-4 w-4" />,         color: "text-emerald-600 bg-emerald-50 border-emerald-200", dot: "bg-emerald-500" },
};
    ...
    <div>
      <div className="flex gap-1 border-b border-zinc-200 mb-6">
        {(["reglas", "templates"] as const).map((t) => (
          ...
          >
            {t === "reglas" ? <Zap className="h-4 w-4" /> : <Mail className="h-4 w-4" />}
            {t === "reglas" ? "Automatizaciones" : "Correos"}
          </button>
        ))}
      </div>

      {tab === "reglas" && <WorkflowTab rules={rules} setRules={setRules} templates={templates} />}
...
function WorkflowTab({ rules, setRules, templates }: {
  ...
  return (
    <div className="space-y-6">
      {/* New rule form */}
      {showForm ? (
        <form onSubmit={createRule} className="border border-blue-200 bg-blue-50 rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-blue-900">Nueva automatización</p>
            <button type="button" onClick={() => setShowForm(false)} className="text-zinc-400 hover:text-zinc-600 text-xs">✕ Cancelar</button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-zinc-600 block mb-1">Nombre</label>
              <Input value={newName} onChange={(e) => setNewName(e.target.value)} required placeholder="ej: Bienvenida tras pago" />
            </div>
            <div>
              <label className="text-xs font-medium text-zinc-600 block mb-1">¿Cuándo se activa?</label>
              <select
                value={newTrigger}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <Input type="number" min={0} value={newDelay} onChange={(e) => setNewDelay(Number(e.target.value))} className="w-20" />
      ...
      {TRIGGER_GROUPS.map((group) => (
        <div key={group.label}>
          <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-3 px-1">{group.label}</p>
...
function WorkflowRow({ trigger, meta, rules, saving, confirmDelete, deleting, onToggle, onDelete, onConfirmDelete }: {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all ${hasRules ? "border-zinc-200 bg-white" : "border-dashed border-zinc-200 bg-zinc-50/50"}`}>
      {/* Trigger row */}
      <div className="flex items-center gap-3 px-4 py-3">
        {/* Event pill */}
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-medium shrink-0 ${meta.color}`}>
          {meta.icon}
          {meta.label}
        </div>
                ...
                <div key={rule.id}>
                  {confirmDelete === rule.id ? (
                    <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                      <p className="text-xs text-red-700 flex-1">¿Eliminar <strong>{rule.name}</strong>?</p>
                      <button onClick={() => onConfirmDelete(null)} className="text-xs text-zinc-500 hover:text-zinc-700 px-2 py-1">Cancelar</button>
                      <button
                    ...
                    <div className={`flex items-center gap-2 rounded-xl px-3 py-2 border transition-colors ${rule.active ? "bg-white border-zinc-200" : "bg-zinc-50 border-zinc-200 opacity-60"}`}>
                      <div className={`w-2 h-2 rounded-full shrink-0 ${rule.active ? meta.dot : "bg-zinc-300"}`} />
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-medium text-zinc-800">{rule.name}</span>
                        <span className="text-xs text-zinc-400 ml-2">{rule.template?.label}</span>
                        {rule.delay_hours > 0 && (
                          <span className="inline-flex items-center gap-1 ml-2 text-[11px] text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">
                            <Clock className="h-2.5 w-2.5" />{rule.delay_hours}h
                          </span>
                        )}
                      </div>
                      <button
  ...
  return (
    <div className="space-y-2">
      {templates.map((t) => {
        ...
        return (
          <div key={t.id} className={`bg-white border rounded-2xl overflow-hidden transition-shadow ${isOpen ? "border-blue-300 shadow-sm" : "border-zinc-200"}`}>
            <div
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full shrink-0 ${t.active ? "bg-emerald-500" : "bg-zinc-300"}`} />
                <div>
                  <p className="text-sm font-medium text-zinc-900">{t.label}</p>
                  <p className="text-xs text-zinc-400 mt-0.5 truncate max-w-sm">{t.subject}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.active ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-500"}`}>
              ...
              <div className="border-t border-zinc-100 p-5 space-y-5">
                ...
                <div className="flex items-center justify-between bg-zinc-50 rounded-xl px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400">Si está inactivo, no se enviará aunque se dispare el evento.</p>
                  </div>
                ...
                {/* Asunto */}
                <div>
                  <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wide block mb-1.5">Asunto</label>
                  <Input
                    value={ed.subject}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">Contenido</label>
                    <button
                  ...
                  {isPreviewing ? (
                    <div className="border border-zinc-200 rounded-xl overflow-hidden">
                      <div className="bg-zinc-100 border-b border-zinc-200 px-3 py-1.5 flex items-center gap-2">
                        ...
                        <p className="text-xs text-zinc-400 mx-auto">Vista previa del correo</p>
                      </div>
                      <iframe
  ...
  return (
    <div className="border border-zinc-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 bg-zinc-50 border-b border-zinc-200 flex-wrap">
        ...
        <ToolbarBtn onClick={setLink} active={editor.isActive("link")} title="Insertar link">
