/**
 * Update: actualiza el template de onboarding en Airtable:
 *  - Reescribe punto 3 (calendario por tandas semanales en vez de 27 de golpe)
 *  - Agrega punto 5 ("marcar como conocido / agregar a contactos")
 *  - Agrega cierre pidiendo respuesta con OK
 *
 * Uso:
 *   npx tsx scripts/update-onboarding-template.ts            # dry-run
 *   npx tsx scripts/update-onboarding-template.ts --apply    # ejecuta
 */
...
<p style="margin:0 0 8px;font-size:15px;font-weight:600;color:#18181b;">Antes del 30 de junio, hay cinco cosas que necesitan hacer para llegar listos al día uno:</p>
<ol style="margin:0 0 16px;padding-left:20px;font-size:15px;color:#52525b;line-height:2;">
  ...
  <li><strong>Las invitaciones llegan por tandas semanales</strong> 📆 — las primeras 2 sesiones esta semana, después vamos sumando las siguientes semana a semana hasta completar las 26. Cero bombardeo, prometido.</li>
  ...
  <li><strong>Marquen <a href="mailto:admin@impacta.vc" style="color:#2563eb;">admin@impacta.vc</a> como conocido</strong> ✅ — la primera vez que les llegue una invitación pueden ver un aviso de "remitente desconocido". Hagan clic en "Sí, conozco al remitente" o agreguen <a href="mailto:admin@impacta.vc" style="color:#2563eb;">admin@impacta.vc</a> a sus contactos. Una vez y listo: las siguientes invites llegan limpias.</li>
</ol>
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log("─".repeat(60));
    console.log("\nPara aplicar:");
    console.log("  npx tsx scripts/update-onboarding-template.ts --apply");
    return;
  }
